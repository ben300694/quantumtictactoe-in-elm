...
xline =
  filled Color.blue (rect 8 (1 / 4 * size))


xcross =
  group [ rotate (degrees 45) xline, rotate (degrees -45) xline ]


ocircle =
  outlined ({ defaultLine | width = 8, color = Color.red }) (circle (size / 10))
...
printClassicalNode { pos, numberOfMove, player } =
  let
    ...
    moveAll =
      move ( -(size / 3) + (toFloat col - 1) * (size / 3), size / 3 - (toFloat row - 1) * (size / 3) )
  ...
  in
    case player of
      ...
      Just X ->
        [ moveAll <| group [ xcross, moveText <| scale 3 <| toForm <| show numberOfMove ] ]

      Just O ->
        [ moveAll <| group [ ocircle, moveText <| scale 3 <| toForm <| show numberOfMove ] ]
...
formatOneField forms =
  let
    moveCoordinates =
      tuples [ -size / 9, 0, size / 9 ] [ size / 9, 0, -size / 9 ]
  in
...
printEntangeledEdge { numberOfMove, player } =
  case player of
    X ->
      scale (1 / 3) <| group [ xcross, move ( size / 9, -size / 9 ) <| scale 3 <| toForm <| show numberOfMove ]

    O ->
      scale (1 / 3) <| group [ ocircle, move ( size / 9, -size / 9 ) <| scale 3 <| toForm <| show numberOfMove ]
...
stateDescription state =
  case state of
    FinishedGame result _ _ ->
      case result of
        Draw ->
          "Game over. Since nobody has three classical marks in the right position it is a Draw"

        Tie p ->
          "Game over. Both players got three classical marks in a line, so we have a tie. 1 Point: " ++ toString p ++ "1/2 Points: " ++ (toString <| switchPlayer p)

        Winner p ->
          "Game over. Winner: " ++ toString p

    NotFinishedGame p _ _ _ ->
      "Next move: " ++ toString p
...
newGameButton =
  button (message newGameMailbox.address ()) "New Game"
  ...
  let
    ...
    collapseMessage =
      if isCollapseNecessary field then
        ...
        "Please insert entangeled marks"
  in
    flow
      down
...
update ( x, y ) ( state, firstClick ) =
  let
    nodeLabel =
      { pos = getPosition ( x, y ), numberOfMove = returnRound state, player = returnPlayer state }
  in
    case firstClick of
      Nothing ->
        case (isCollapseNecessary <| returnField state) of
          True ->
            ( addMove (Collapse { id = (idFromPosition <| getPosition ( x, y )), label = nodeLabel }) state, Nothing )

          False ->
            ( state, Just ( x, y ) )

      Just ( xold, yold ) ->
        ( addMove (Entangled { first = idFromPosition <| getPosition ( x, y ), second = idFromPosition <| getPosition ( xold, yold ), label = { numberOfMove = returnRound state, player = unsafe <| returnPlayer state } }) state, Nothing )
...
idFromPosition { col, row } =
  3 * (col - 1) + row


gameStateSignal : Signal ( GameState, Maybe ( Int, Int ) )
gameStateSignal =
  Signal.foldp update ( initialState, Nothing ) clickSignal
...
testNode2 =
  { id = 4, label = { pos = { col = 2, row = 1 }, numberOfMove = -1, player = Nothing } }


testNode3 =
  { id = 6, label = { pos = { col = 2, row = 3 }, numberOfMove = -1, player = Nothing } }
