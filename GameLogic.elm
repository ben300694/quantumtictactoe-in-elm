...
isValidMove move state =
  ...
  in
    case move of
      Entangled edge ->
        if (isCollapseNecessary field) || (edge.first == edge.second) then
          False
        else
...
replaceNode field node newPlayer = let
                  (nodes, edges) = field
                  newNumberOfMove = case maximum
                                        <| List.map (\x -> x.label.numberOfMove)
                                        <| List.filter (\x -> x.label.player == newPlayer) (incidentEdges field node)
                                    of Nothing -> -1
                                       Just n -> n
                  oldLabel = node.label
                  newLabel = {oldLabel | numberOfMove = newNumberOfMove, player = Just newPlayer}
                  newNode = {node | label = newLabel}
  in
    ( updateNode newNode nodes, edges )

setPlayerInNode : Field -> Node NodeLabel -> Player -> Round -> Field
setPlayerInNode field node newPlayer round = let
                  (nodes, edges) = field
                  oldLabel = node.label
                  newLabel = {oldLabel | numberOfMove = round, player = Just newPlayer}
                  newNode = {node | label = newLabel}
  in
    (updateNode newNode nodes, edges)
...
performOneStepInCollapse field = let
    (nodes,edges) = field
  in
...
collapseOneEdge field edge = let
  ...
  in if not <| isEdgeInGraph field edge then field else
    case firstNode.label.player of
        Nothing -> case secondNode.label.player of
                        Nothing -> field
                        Just player -> if player == edgePlayer && edge.label.numberOfMove == secondNode.label.numberOfMove
                                       then removeEdge field edge 
                                       else removeEdge (setPlayerInNode field firstNode edgePlayer edge.label.numberOfMove) edge
        Just player -> case secondNode.label.player of
