...
removeDuplicates list = case list of
        [] -> []
        (l::ls) -> if (List.member l ls) then (removeDuplicates ls) else (l :: (removeDuplicates ls) )
...
unsafe v = case v of 
        Just x -> x
        Nothing -> Debug.crash "unsafe with Nothing"
...
type alias Graph a e = (List (Node a), List (Edge e))

getNodeFromId : Graph a e -> NodeId -> Maybe (Node a)
getNodeFromId graph nodeid = let
    (nodes, edges) = graph
  in
    head <| filter (\x -> x.id == nodeid) nodes

incidentEdges : Graph a e -> Node a -> List (Edge e)
incidentEdges graph {id} = let
    (_, edges) = graph
    f = \x -> x.first == id || x.second == id
  in
    List.filter f edges

adjacentNodes : Graph a e -> Node a -> List (Node a)
adjacentNodes graph node = let
    (nodes, edges) = graph
  in
    List.map unsafe <| filter isJust <| List.map (getNodeFromId graph) <| filter (\x -> if (x /= node.id) then True else False) <| removeDuplicates <| List.concatMap (\ed -> [ed.first, ed.second]) (incidentEdges graph node)    
...
findCycle graph = Nothing {-TODO-}

{-Gives all nodes reachable frome one specific node-}
reachableFromNode : Graph a e -> Node a -> List (Node a)
reachableFromNode graph node = reachableFromNode' graph node []

reachableFromNode' : Graph a e -> Node a -> List (Node a) -> List (Node a)
reachableFromNode' graph node visited = if (List.member node visited) then visited else
    case List.filter (\x -> not (List.member x visited)) (adjacentNodes graph node) of
        [] -> (node::visited)
        rest -> visitfold graph rest (node::visited) 

visitfold graph list visited = case list of
        [] -> visited
        (n::ns) -> visitfold graph ns (reachableFromNode' graph n visited)
...
reachableFromComponent graph nodes = case nodes of
        [] -> []
        (n::ns) -> reachableFromNode graph n
...
type alias Field = Graph NodeLabel EdgeLabel
...
-}
type Move = Entangled (Edge EdgeLabel) | Collapse (Node NodeLabel)
type alias Moves = List Move
type alias Round = Int

type GameState = FinishedGame Result Field Moves | NotFinishedGame Player Field Moves Round

returnField : GameState -> Field
returnField state = case state of
        FinishedGame _ field _ -> field
        NotFinishedGame _ field _ _ -> field

returnRound : GameState -> Round
returnRound state = case state of
        FinishedGame _ _ _ -> 0
        NotFinishedGame _ _ _ n -> n

returnPlayer : GameState -> Maybe Player
returnPlayer state = case state of
        FinishedGame _ _ _ -> Nothing
        NotFinishedGame player _ _ _ -> Just player

returnMoves : GameState -> Moves
returnMoves state = case state of
        FinishedGame _ _ moves -> moves
        NotFinishedGame _ _ moves _ -> moves
...
nodeConstr (i,(x,y)) = { id = i, label = { pos = { col = x, row = y }, numberOfMove = -1, player = Nothing }}

zip : List a -> List b -> List (a,b)
zip = List.map2 (,)

tuples : List a -> List b -> List (a,b)
tuples xs ys = List.concatMap (\x -> (List.map (\y -> (x,y)) ys)) xs

fieldNumbers = tuples [1..3] [1..3]
nodes = List.map nodeConstr (zip [1..9] fieldNumbers)
...
initialState = NotFinishedGame X emptyField [] 0

isEdgeOnField : Edge EdgeLabel -> Field -> Bool
isEdgeOnField edge field = let
    (nodes, edges) = field
    condition = \x -> ((x.first == edge.first && x.second == edge.second) || (x.first == edge.second && x.second == edge.first)) && edge.label.player == x.label.player
  in
...
isValidMove move state = let
    field = returnField state
    (nodes, edges) = field
  in
    case move of
            Entangled edge -> if (isCollapseNecessary field) then False else not <| isEdgeOnField edge field
            Collapse node -> if (isCollapseNecessary field)
                then let
                        cycle = unsafe <| findCycle field
                        (cyclenodes, cycleedges) = cycle
                        potentialCollapsePoints = reachableFromComponent field cyclenodes
                        edgesIncidentToNode = incidentEdges field node
                     in
                        if member node.id (List.map .id potentialCollapsePoints) && member (unsafe node.label.player) (List.map (\y  -> y.label.player) edgesIncidentToNode) then True else False 
                else False

{-check for a circle in the graph and then return if this collapse is possible-}
isCollapseNecessary : Field -> Bool
isCollapseNecessary field = isJust <| findCycle field
...
addMove move state = let
    field = returnField state
    (nodes, edges) = field
  in
    if (isValidMove move state)
        then checkIfFinished <| case move of
                Entangled edge -> NotFinishedGame (switchPlayer <| unsafe <| returnPlayer state) (nodes, {edge | label = {numberOfMove = returnRound state, player = unsafe <| returnPlayer state}} :: edges) (move :: returnMoves state) ((returnRound state) + 1) 
                Collapse node -> NotFinishedGame (unsafe <| returnPlayer state) (updateNode node nodes,edges) (move :: returnMoves state) (returnRound state)
        else state 

{-Updates the node in the list with the right id to the new one-}
updateNode : Node a -> List (Node a) -> List (Node a)
updateNode node nodes = List.map (\x -> if x.id == node.id then node else x) nodes

--type alias NodeLabel = { pos: Position, numberOfMove: Int, player: Maybe Player }
--type alias EdgeLabel = { numberOfMove: Int, player: Player }
--type GameState = FinishedGame Result Field Moves | NotFinishedGame Player Field Moves Round
checkIfFinished : GameState -> GameState
checkIfFinished state = case state of
        FinishedGame result field moves -> FinishedGame result field moves
        NotFinishedGame player field moves round -> NotFinishedGame player field moves round {-TODO-} 

{--}
--performEntireCollapse : Field -> Field
performEntireCollapse field = field {-TODO-}
